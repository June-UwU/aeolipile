...
inline const char *vkresult_to_string(VkResult result) {
  ...
  case VK_PIPELINE_BINARY_MISSING_KHR:
    return "The application attempted to create a pipeline binary by querying "
           "an internal cache, but the internal cache entry did not exist";
  ...
  case VK_ERROR_INVALID_DEVICE_ADDRESS_EXT:
    return "A buffer creation failed because the requested address is not "
           "available";
    //  case VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS:
    // return "A buffer creation, memory allocation, or shader group handle
    // assignment failed because the requested address is not available";
  case VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT:
    return "An operation on a swapchain created with exclusive control failed "
           "as it did not have exclusive full-screen access";
  case VK_ERROR_PRESENT_TIMING_QUEUE_FULL_EXT:
    return "Insufficient space in the swapchain's internal results queue to "
           "hold requested present timing data";
  ...
  case VK_ERROR_VIDEO_PROFILE_OPERATION_NOT_SUPPORTED_KHR:
    return "A video profile operation specified is not supported";
  case VK_ERROR_VIDEO_PROFILE_FORMAT_NOT_SUPPORTED_KHR:
    return "Format parameters in a requested video profile chain are not "
           "supported";
  case VK_ERROR_VIDEO_PROFILE_CODEC_NOT_SUPPORTED_KHR:
    return "Codec-specific parameters in a requested video profile chain are "
           "not supported";
  case VK_ERROR_VIDEO_STD_VERSION_NOT_SUPPORTED_KHR:
    return "The specified video Std header version is not supported";
  case VK_ERROR_INVALID_VIDEO_STD_PARAMETERS_KHR:
    return "The specified Video Std parameters do not adhere to the syntactic "
           "or semantic requirements";
  case VK_ERROR_NOT_PERMITTED:
    return "The driver implementation has denied a request to acquire a "
...
#define VK_ASSERT(predicate, format, ...)                                      \
  {                                                                            \
    VkResult result = (predicate);                                             \
    if (VK_SUCCESS != result) {                                                \
      TLOGF("Vulkan Error : %s\n", vkresult_to_string(result));                \
