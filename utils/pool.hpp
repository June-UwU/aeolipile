...
public:
  ...
  handle_t create(object_t &&obj) {
    ...
    if (free_list.empty()) {
      ...
      object_pool.push_back({std::move(obj), 1});

      index_t end = object_pool.size();
      populate_free_list(index + 1, end);
    } else {
      index = free_list.top();
      object_pool[index] = {std::move(obj), 1};
      free_list.pop();
    }
  ...
  void destroy(handle_t handle) {
    ...
    if (index >= object_pool.size() || index < 0) {
      return;
    }

    handle_t current_handle(index, object_pool[index].second);
  ...
  const object_t *get(handle_t handle) {
    ...
    if (object_pool[index].second != handle.gen()) {
      return nullptr;
    }
  ...
  handle_t update_generation(handle_t handle) {
    ...
    return handle_t(index, object_pool[index].second);
  }
